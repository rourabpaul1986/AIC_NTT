...
def CT_Butterfly(i, j, n, x_out, A, twiddle_factor, M, length, f, l, w, array, tw_index, eqs, err, no_err,  c, injection, mode, verbose):
    ...
    for k in range(halflen):
        ...
        v_dash=  mont_mult_top(l, w, A[k + halflen], twiddle_factor, M, f, verbose=False)
        ###########################fault mode################################
        #print(f"v before fault: {v}")
        #print(f"injection type: {injection}")
        
        ###########################fault mode################################
        v, u_plus_v, u_minus_v =inject_fault_full(injection, mode, v_mult,  u, v_dash, l, f, M)
        #v_mult, u_plus_v, u_minus_v =inject_fault_full(injection, mode, v_mult,  u, l, f, M)
        #####################################################################
        if verbose:
        ...
        #error, no_error=AIC(u+v, u-v_dash, u, v, v_dash, M, k, err, no_err, verbose)
...
class fNTT:
    ...
    def forward(self, x_in, N, f, l, w, eqs_f, err, no_err, injection, mode, verbose=False):
        if len(x_in) != self.N:
        ...
        for i in range(self.Nlen):
            ...
            for j in range(n):
                ...
                array, err, no_err, c= CT_Butterfly(i, j, n, x[seqlen * j: seqlen * (j + 1)],
                                     x[seqlen * j: seqlen * (j + 1)],
                                     twiddle_factor, self.M, seqlen, f, l, w, array, tw_index, eqs_f, err, no_err,  c, injection, mode, verbose)
        #print(f"c:{c}")
        return bit_reverse([i % self.M for i in x], self.Nlen), err, no_err

    def inverse(self, x_in, N, f, l, w, eqs_r, err, no_err, injection, mode, verbose=False):
        if len(x_in) != self.N:
            raise ValueError(f'Input should be sized {self.N}')
        x = np.copy(x_in)
        array = N * [0]
        ...
        for i in range(self.Nlen):
            ...
            for j in range(n):
                ...
                array, err, no_err, c = CT_Butterfly(i, j, n, x[seqlen * j: seqlen * (j + 1)],
                                     x[seqlen * j: seqlen * (j + 1)],
                                     twiddle_factor, self.M, seqlen, f, l, w, array, tw_index, eqs_r, err, no_err, c, injection, mode, verbose)
...
def check_arrays(array1, array2, verbose):
    ...
    for i in range(len(array1)):
        if array1[i] != array2[i]:
            print(f"F Wrong: Elements at index {i} are different ({array1[i]} != {array2[i]}).")
            return False
...
if __name__ == "__main__":
    ...
    for i in range(sample_size):
        ...
        ntt_A, err, no_err = ntt.forward(A, N, f, l, w, eqs_f, err, no_err, injection, mode, verbose=verbose)
        sum=sum_of_array(ntt_A)
        #if verbose:
         #print(f"Forward NTT of A: {ntt_A}\n")
        #print(f"FError: {err}\n")
        if f==0:
            inv_ntt_A, err, no_err = ntt.inverse(ntt_A, N, f, l, w, eqs_r, err, no_err, injection, mode, verbose=verbose)
            if verbose:
                print(f"Reversed NTT of A: {inv_ntt_A}")
            check_arrays(inv_ntt_A, A, verbose)
        #print(f"RError: {err}\n")
    
    print(f"Effciency for Full AIC: {100*err/(sample_size*(math.log2(N)*(N/2)))}%\n")
